...
def main_menu(conn, cursor):
    ...
    def create_recipe(conn, cursor):
        ...
        for i in range(0, n):
            ingredient = input(' - ')
            # adding the ingredient to ingredients list
    ...
    def search_recipe(conn, cursor):
        ...
        search_ingredient = int(
            input('Enter the corresponding number of the ingredient you want to search:   '))
        ...
        sql = "SELECT ingredients FROM Recipes WHERE ingredients LIKE '%" + \
            search_ingredient + "%'"
        cursor.execute(sql)
        results_3 = cursor.fetchall()
        cursor.execute(
            "SELECT name FROM Recipes WHERE ingredients LIKE '%" + search_ingredient + "%'")
    ...
    def update_recipe(conn, cursor):
        ...
        searched_recipe_id = str(
            input('Enter the corresponding number of the recipes you want to update:   '))

        if not searched_recipe_id == None:
            searched_recipe_column = str(input(
                "What column do you want to update? Type 'name', 'cooking_time' or 'ingredients':   "))

            if searched_recipe_column == 'name':
                new_name = str(input('Enter a new name for this recipe:  '))
                sql = "UPDATE Recipes SET name = '" + new_name + \
                    "' WHERE id = " + searched_recipe_id
                cursor.execute(sql)
                print("\nUpdated name to: " + new_name)

            elif searched_recipe_column == 'cooking_time':
                new_cookingtime = str(
                    input('Enter a new cooking time for this recipe:   '))
                new_difficulty = calculate_difficulty(
                    int(new_cookingtime), row[4])
                sql = "UPDATE Recipes SET cooking_time = " + \
                    new_cookingtime + " WHERE id = " + searched_recipe_id
                sql_2 = "UPDATE Recipes SET difficulty = '" + \
                    new_difficulty + "' WHERE id = " + searched_recipe_id
                cursor.execute(sql)
                cursor.execute(sql_2)
                print("\nUpdated cooking time to: " + new_cookingtime +
                      ", and difficulty to: " + new_difficulty)
                ...
                sql = "UPDATE Recipes SET ingredients = '" + \
                    new_ingredients_joined + "' WHERE id = " + searched_recipe_id
                sql_2 = "UPDATE Recipes SET difficulty = '" + \
                    new_difficulty + "' WHERE id = " + searched_recipe_id
                cursor.execute(sql)
                cursor.execute(sql_2)
                print("\nUpdated Ingredients to: " + new_ingredients_joined +
                      ", and difficulty to: " + new_difficulty)
    ...
    def delete_recipe(conn, cursor):
        ...
        searched_recipe_id = str(
            input('Enter the corresponding number of the recipes you want to delete:   '))
        try:
